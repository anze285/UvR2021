...
def podatek(kljuc, podatki):
    izpis = None
    for podatek1, podatek2 in podatki:
        if (podatek1 == kljuc):
            izpis = podatek2
    return izpis
...
def razdalja(t1, t2):
    t11, t12 = t1
    t21, t22 = t2
    vsota = (t11-t21)**2 + (t12 - t22)**2
    vsota = vsota**0.5
    return vsota

def oddaljenost(kraj1, kraj2, koordinate):
    koordinata1 = podatek(kraj1, koordinate)
    koordinata2 = podatek(kraj2, koordinate)
    return (razdalja(koordinata1,koordinata2))

def visina(kraj1, kraj2, visine):
    visina1 = podatek(kraj1, visine)
    visina2 = podatek(kraj2, visine)
    return (visina2 - visina1)
...
def skupni_cas(kraji, koordinate, visine):
    ...
    for kraj in kraji:
        if p_kraj != None:
            all_time += cas_med(p_kraj, kraj, koordinate, visine)
        p_kraj = kraj
    return all_time

def skrajsaj(kraji, koordinate, visine):
    najmanjsi_cas = None
    for idx, kraj in enumerate(kraji):
        kraji1 = kraji[:]
        kraji1.pop(idx)
        skupni_cas_krajev = skupni_cas(kraji1, koordinate, visine)
        if najmanjsi_cas == None:
            najmanjsi_cas = skupni_cas_krajev
            kraji2 = kraji1[:]
        elif najmanjsi_cas >= skupni_cas_krajev:
            najmanjsi_cas = skupni_cas_krajev
            kraji2 = kraji1[:]
    return kraji2
